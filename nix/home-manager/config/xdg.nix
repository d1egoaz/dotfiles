...
let
  ...
  # Personal Codex runs on the OpenCode Go subscription. The desktop app has no
  # profile selector, so this is the base config both the app and the CLI read.
  codexLeadModel = if profile == "office" then "gpt-5.6-sol" else "deepseek-v4.1-flash";
  # Office keeps the deepest lead effort; the personal OpenCode Go route runs at
  # high, which is the owner's preference for DeepSeek V4.1 Flash.
  codexLeadReasoningEffort = if profile == "office" then "xhigh" else "high";
  codexOpenCodeGo = profile != "office";
in
{
  # ============================================================================
  ...
  #
  # This stays a runtime activation script on purpose: `work.local.toml` is
  # gitignored and `config/` sits outside the flake root (`nix/`), so pure
  # evaluation (`just check`, `just dry-run`) could never read these files at
  # build time. The generated target is disposable; only a hand-written
  ...
  home.activation.codexConfigProfile = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    ...
    CODEX_LEAD_MODEL="${codexLeadModel}"
    CODEX_LEAD_REASONING_EFFORT="${codexLeadReasoningEffort}"
    CODEX_OPENCODE_GO="${if codexOpenCodeGo then "1" else "0"}"
    ...
    # Generate the model catalog before composing the config. A missing catalog
    # path is fatal to Codex, so the key is only written when the file exists;
    # without it Codex falls back to default model metadata.
    CATALOG="$HOME/.codex/ocg-catalog.json"
    CATALOG_TOOL="$HOME/dotfiles/bin/files/codex-opencode-go-catalog"
    if [ "$CODEX_OPENCODE_GO" = "1" ]; then
      if [ -x "$CATALOG_TOOL" ]; then
        "$CATALOG_TOOL" ||
          echo "Could not generate $CATALOG; Codex will start without it" >&2
      else
        echo "Missing $CATALOG_TOOL; Codex will start without a model catalog" >&2
      fi
    fi
    ...
    rm -f "$TMP"
    {
      ...
      # Root keys must stay above every table. Personal machines route through
      # the local codex-gateway, whose catalog offers both the OpenCode Go models
      # and the plan's own models, so no provider switching is needed.
      printf 'model = "%s"\n' "$CODEX_LEAD_MODEL"
    ...
    # One-time migration: preserve a hand-written config before overwriting.
    if [ -f "$TARGET" ] && [ ! -L "$TARGET" ] \
      && ! head -n 1 "$TARGET" | grep -q '^# Generated by Home Manager from dotfiles Codex profile:'; then
      BACKUP="$TARGET.backup.$(date -u +%Y%m%dT%H%M%SZ)"
      cp "$TARGET" "$BACKUP"
      echo "Backed up hand-written $TARGET to $BACKUP" >&2
    fi
