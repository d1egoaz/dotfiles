...
in
{
  ...
  home.activation.codexConfigProfile = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    ...
    # The model catalog is generated by `just codex-catalog`, which also
    # restarts the app server. Activation only points at it when it already
    # exists: a missing catalog path is fatal to Codex, so without the file the
    # key is omitted and Codex falls back to default model metadata.
