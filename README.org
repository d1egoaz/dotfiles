...
- *office-mbp* — Office MacBook Pro M1 Max (user: diego.albeiroalvarezzuluag)
...
Encrypted with sops-nix. Requires =sops= and =gnupg= installed:
...
#+begin_src bash
sops nix/profiles/office.private.sops.json
#+end_src

Secrets are decrypted at runtime and never stored in plaintext. Configuration is in =.sops.yaml= with your GPG fingerprint.
...
# Check GPG key is properly configured and imported
