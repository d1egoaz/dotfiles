...
*Note:* Uses nixpkgs 25.11-darwin (stable) for reliability — avoiding constant updates for basic commands.
...
just audit         # Compare this Mac with dotfiles (read-only)
just audit-export  # Write a sanitized audit JSON to the Desktop
just --list        # See all available commands
#+end_src

=just audit= exits with =0= for a complete match, =1= when drift is found, and =2= when any audit coverage is unavailable. =just audit-export= always writes the available sanitized evidence when collection succeeds, including explicit coverage gaps. It excludes secrets, environment variables, network details, browser data, hardware identifiers, file contents, Git diffs, and full macOS defaults exports.

** Model routing

Model tiers and profile mappings live in =nix/data/agent-routing.toml=, the only
hand-edited routing source. After changing it, regenerate the Codex roles and
runtime data, then verify there is no generated drift:

#+begin_src bash
just agent-routing-generate
just agent-routing-check
#+end_src

The stable native-subagent launchers are =@spawn-subagent-economy=,
=@spawn-subagent-balanced=, and =@spawn-subagent-frontier=. They pin only the
capability tier; the parent handoff supplies the outcome and constraints. The
specialized launchers add a fixed role contract, such as
=@spawn-subagent-balanced-review=. These are native subagent types, not visible
child tasks. Their public names stay stable when the model mapping changes;
spawned instance names include the resolved model label for historical
traceability.
...
Secrets follow an on-demand model — nothing is exported into every shell at startup.

- *1Password CLI* is the source of truth for raw secrets. Fish exposes on-demand aliases (=op-cursor=, and =op-homebrew= on office machines); =just brew= / =just update= read the Homebrew GitHub token directly from 1Password when needed.
- *=op-env-cache=* caches 1Password reads on disk (0600, default 8h TTL) for hot paths like the Alfred LLM workflows. The =op://= item references it resolves live encrypted in =secrets/op-env-cache.yaml= under =op_env_cache_specs=.
- *SOPS (age)* encrypts =secrets/*.yaml=; the age key lives at =~/Library/Application Support/sops/age/keys.txt= and is backed up in 1Password. Work-only ignored files (Codex work profile, hooks, rules, skills) are persisted as encrypted =local_state= entries in =secrets/local-state.yaml= and restored by =just switch= on office machines; push local edits back with =just local-state-sync=.
...
2. *Direct files* — Drop configs in =config/<app>/= and link in =home-manager/config/xdg.nix=
   - Example: =config/wezterm/=, =config/ghostty/=, =config/aerospace/=
