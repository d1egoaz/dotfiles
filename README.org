...
The native-subagent roles are =@spawn-subagent-explore= (read-only),
=@spawn-subagent-implement= (workspace-write), and =@spawn-subagent-review=
(read-only). The registry sets each role's tier and effort, so names stay
stable when routing changes; spawned instance names include the resolved model
label for historical traceability. These are native subagent types, not visible
child tasks.
