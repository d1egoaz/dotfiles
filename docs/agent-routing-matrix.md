...
|---|---|---|---|---|---|---|
| Codex (Office Codex) | `gpt-6-sol` / high | `gpt-6-luna` / high | `gpt-6-sol` / low | `gpt-6-sol` / low | `gpt-6-luna` / xhigh | `gpt-6-luna` / xhigh |
| Claude Code (Office Claude Code) | `opus` (`claude-opus-5-5`) / high | `sonnet` (`claude-sonnet-5`) / high | `opus` (`claude-opus-5-5`) / low | `opus` (`claude-opus-5-5`) / low | `sonnet` (`claude-sonnet-5`) / xhigh | `sonnet` (`claude-sonnet-5`) / session effort |

## personal

| Harness | Lead | `spawn-subagent-gather` | `spawn-subagent-explore` | `spawn-subagent-review` | `spawn-subagent-implement` | Unnamed spawn |
|---|---|---|---|---|---|---|
| Codex (Personal Codex home tiers) | `xiaomi/mimo-v2.6-pro` / high | `deepseek-v4.1-flash` / high | `deepseek-v4-pro` / high | `deepseek-v4-pro` / high | `deepseek-v4.1-flash` / xhigh | `deepseek-v4.1-flash` / xhigh |
