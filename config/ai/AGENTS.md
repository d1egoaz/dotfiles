...
- Do not interrupt a running subagent merely because it is unfinished or the lead needs more information. First inspect its status, send a focused status request or redirect through the parent-to-child communication path, and wait for a bounded response. Ask it to report completed work, its current action, blockers, and remaining work, and to pause before additional writes when a decision is needed.
- Interrupt only when continued execution is unsafe, the scope has been invalidated, writers conflict, failures repeat without progress, the user explicitly cancels the work, or another concrete reason makes continued execution undesirable. If an agent is idle and more work is needed, send a follow-up instead of creating a duplicate.
