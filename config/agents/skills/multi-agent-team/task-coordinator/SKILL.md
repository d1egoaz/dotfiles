...
- Before interrupting a running subagent, inspect its status, send a focused status request or redirect through the runtime's parent-to-child control, and wait for a bounded response. Ask for completed work, the current action, blockers, remaining work, and a pause before additional writes when a decision is needed. A long-running tool call may delay delivery until the next safe message boundary; that delay alone is not a reason to interrupt.
- Interrupt only for unsafe work, invalidated scope, conflicting writers, repeated failure without progress, explicit user cancellation, or another concrete reason continued execution is undesirable. If the subagent is idle and more work is needed, send a follow-up or resume it instead of spawning a duplicate.

### Codex Control Mapping

- Treat the names in this section as model-visible Codex runtime tools, not lifecycle events or universal public API methods. Use them only when the current host exposes them; use the equivalent native controls in other runtimes.
- For native subagents, use `list_agents` to inspect current state, `send_message` to query or redirect a running agent, `wait_agent` to wait for a mailbox update, and `followup_task` to start more work when an agent is idle. Use `interrupt_agent` only for the interruption reasons above. Prefer `send_message` for steering the current run; a message may wait for the next safe boundary when a tool call is active.
- For visible Codex tasks, use `wait_threads` for bounded progress waits, `read_thread` for detailed state and results, and `send_message_to_thread` for a visible follow-up. Do not open a duplicate task merely to obtain status or deliver new context.
- For a direct Codex App Server integration, the approximate public method mapping is `thread/read` for inspection, `turn/steer` for input during an active turn, `turn/start` for a new turn, and `turn/interrupt` for cancellation. Lifecycle notifications include `thread/started`, `item/started`, `item/completed`, and `turn/completed`. Do not attempt to call App Server method names as model tools unless the runtime actually exposes that API.
