...
- Keep ignored local state durable by syncing encrypted `local_state` entries in `secrets/local-state.yaml` with `just local-state-sync` or `bin/files/dotfiles-local-state sync`; `just switch` only restores missing files. Restored files are plaintext copies on disk, not runtime-decrypted files.
...
- Office machines restore ignored work-local skills from encrypted `secrets/local-state.yaml` when `config/agents/skills/work.local` is missing or empty.
